...
void main() {
  ...
  test(() {
    ...
    assert_equals(
        input.selectionStart, 0, "element.selectionStart should be 0");
  ...
  test(() {
    ...
    assert_equals(input.selectionEnd, input.value.length,
        "Arguments greater than the length of the value of the text field must be treated as pointing at the end of the text field");
  }, 'input setSelectionRange(0,input.value.length+1)');
  ...
  test(() {
    ...
    assert_equals(input.selectionDirection, "backward",
        'The direction of the selection must be set to backward if direction is a case-sensitive match for the string "backward"');
  }, 'input direction of setSelectionRange(0,1,"backward")');

  test(() {
    input.setSelectionRange(0, 1, "forward");
    assert_equals(input.selectionDirection, "forward",
        'The direction of the selection must be set to forward if direction is a case-sensitive match for the string "forward"');
  }, 'input direction of setSelectionRange(0,1,"forward")');

  test(() {
    input.setSelectionRange(0, 1, "none");
    assert_equals(input.selectionDirection, "none",
        'The direction of the selection must be set to forward if direction is a case-sensitive match for the string "none"');
  ...
  test(() {
    input.setSelectionRange(0, 1, "BACKWARD");
    assert_equals(input.selectionDirection, "none",
        "selectionDirection should be 'none'");
  }, 'input direction of setSelectionRange(0,1,"BACKWARD")');

  test(() {
    input.setSelectionRange(0, 1);
    assert_equals(
        input.selectionDirection, "none", "if the argument is omitted");
  ...
  test(() {
    ...
    assert_equals(
        textarea.selectionStart, 0, "element.selectionStart should be 0");
  ...
  test(() {
    textarea.setSelectionRange(0, 1);
    assert_equals(
        textarea.selectionDirection, "none", "if the argument is omitted");
