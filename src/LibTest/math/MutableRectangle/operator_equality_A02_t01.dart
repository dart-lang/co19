...
main() {
  MutableRectangle r1=new MutableRectangle(0, double.MAX_FINITE, double.INFINITY, double.NAN);
  for (int i = 0; i < values.length; i++) {
    for (int j = 0; j < values.length; j++) {
      for (int k = 0; k < values.length; k++) {
        for (int l = 0; l < values.length; l++) {
          MutableRectangle r2=new MutableRectangle(values[i], values[j], values[k], values[l]);
    	  var res = (r1==r2);
