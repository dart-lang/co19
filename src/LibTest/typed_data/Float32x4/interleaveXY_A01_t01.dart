 ...
 */
/**
 * @assertion Float32x4 interleaveXY(Float32x4 other)
 * Returns a new [Float32x4] with the X and Y lane values
 * from [this] and [other] interleaved.
 * @description Checks that the returned value contains interleaved X and Y lane
...
check(thisObj, otherObj) {
  var res = thisObj.interleaveXY(otherObj);
  Expect.isFalse(res == thisObj);
  Expect.isFalse(res == otherObj);
  Expect.equals(thisObj.x, res.x);
  Expect.equals(otherObj.x, res.y);
  Expect.equals(thisObj.y, res.z);
