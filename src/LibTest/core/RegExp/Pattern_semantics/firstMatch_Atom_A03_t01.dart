...
main() {
  check(r"([Jj]ava([Ss]cript)?)\sis\s(fun\w*)", "Learning javaScript is funny, really", 9, ["javaScript is funny", "javaScript", "Script", "funny"]);
  check(r"([Jj]ava([Ss]cript)?)\sis\s(fun\w*)", "Developing with Java is fun, try it", 16, ["Java is fun", "Java", null, "fun"]);
  ...
  check(r"(a(b(c)))(d(e(f)))", "xabcdefg", 1, ["abcdef","abc","bc","c","def","ef","f"]);
  check(r"(a(b(c)))(d(e(f)))\2\5", "xabcdefbcefg", 1, ["abcdefbcef","abc","bc","c","def","ef","f"]);
  check(r"a(.?)b\1c\1d\1", "abcd", 0, ["abcd", ""]);
  check(r"([\S]+([ \t]+[\S]+)*)[ \t]*=[ \t]*[\S]+", "Course_Creator = Test", 0, ["Course_Creator = Test", "Course_Creator", null]);
  check(r"^(A)?(A.*)$", "AAA", 0, ["AAA", "A", "AA"]);
  check(r"^(A)?(A.*)$", "AA", 0, ["AA", "A", "A"]);
  check(r"^(A)?(A.*)$", "A", 0, ["A", null, "A"]);
  check(r"(A)?(A.*)", "zxcasd;fl\\  ^AAAaaAAaaaf;lrlrzs", 13, ["AAAaaAAaaaf;lrlrzs", "A", "AAaaAAaaaf;lrlrzs"]);
  check(r"(A)?(A.*)", "zxcasd;fl\\  ^AAaaAAaaaf;lrlrzs", 13, ["AAaaAAaaaf;lrlrzs", "A", "AaaAAaaaf;lrlrzs"]);
  check(r"(A)?(A.*)", "zxcasd;fl\\  ^AaaAAaaaf;lrlrzs", 13, ["AaaAAaaaf;lrlrzs", null, "AaaAAaaaf;lrlrzs"]);
  ...
  check(r"^(([a-z]+)*[a-z]\.)+[a-z]{2,}$", "www.netscape.com", 0, ["www.netscape.com", "netscape.", "netscap"]);
  check(r"^(([a-z]+)*([a-z])\.)+[a-z]{2,}$", "www.netscape.com", 0, ["www.netscape.com", "netscape.", "netscap", "e"]);
}
