...
void check(int d, int h, int m, int s, int ms, num f) {
  Expect.equals((d*f) * 24 * 3600000 + (h*f) * 3600000 + (m*f) * 60000 + (s*f) * 1000 + ms*f, 
      (new Duration(days: d, hours: h, minutes: m, seconds: s, milliseconds: ms) * f
      ).inMilliseconds);
}
...
main() {
  for (int k=0; k<100; k++) {
    check(nextRand(), nextRand(), nextRand(), nextRand(), nextRand(), nextRand());
  }
}
