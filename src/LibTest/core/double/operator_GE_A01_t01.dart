 ...
 * @description Checks operator >= for two doubles (double special values, adjacent sample doubles).
 * @author pagolubev
 * @reviewer msyabro
 */
...
final List sample_doubles = const [
   -1/0, -1.7976931348623157e308, -3.273390607896142E150, -4.503599627370497E15, -4.503599627370496E15,
...
main() {
  for(int i = 0; i < sample_doubles.length; i++) {
     double min = sample_doubles[i];
     Expect.isTrue(min >= min, "Expected $min >= $min");
     for(int j = i + 1; j < sample_doubles.length; j++) {
       double max = sample_doubles[j];
       Expect.isTrue(max >= min, "Expected $max >= $min");
       Expect.isFalse(min >= max, "Expected !($min >= $max)");
     }
  }
}
