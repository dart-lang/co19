...
class TestIsolate extends Isolate {
   ...
   void act(var message, SendPort replyTo) {
      void reply(var m, SendPort fromBob) {
        if (m == "Ok") {
      ...
      if (message == "Spawn") {
        ...
        this.spawn().then(void func(SendPort portB) {
          //portB.call("Your name is Bob").then(reply);
          var receiveFromBobPort = new ReceivePort();
          portB.send("Your name is Bob", receiveFromBobPort.toSendPort());
          receiveFromBobPort.receive(reply);
        });
...
void main() {
  new TestIsolate().spawn().then(void func(SendPort toAlice) {
    ...
    fromAlice.receive( void foo(var message, SendPort toBob) {
      if(message == "Spawned") {
    ...
    fromBob.receive( void foo(var message, SendPort unused){
      if(message == "Bob") {
