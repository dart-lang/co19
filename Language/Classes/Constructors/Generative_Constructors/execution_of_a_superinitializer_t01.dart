 ...
 */
/**
 * @assertion Execution of a superinitializer of the form super(a1, ..., an; xn+1: an+1,...,xn+k: an+k)
 ...
 * @description Checks that the execution order as well as type parameters binding for
 * both named and unnamed superinitializers are correct.
 * @author iefremov
 * @reviewer rodionov
 */
...
class S<T, U> {
  S(var x, var y, [var o]) : this.s1 = x("s1${1 is T}"), this.s2 = y("s2${1.0 is U}"),
...
main() {
  log = [];
  new C();
  Expect.listEquals(["a1","a2", "o", "s1true", "s2true", "false", "false", "ok"], log, "Wrong superinitializer evaluation order!");

  log = [];
  new C.named();
  Expect.listEquals(["a1","a2", "o", "s1true", "s2true", "false", "false", "named!"], log, "Wrong superinitializer evaluation order!");
