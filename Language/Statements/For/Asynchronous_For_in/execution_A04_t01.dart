...
///
/// @description Check that if execution of `s` continues without a label then
/// the execution of `s` is treated as if it had completed normally
///
...
main() async {
  List<int> log = [];
  await for(var i in Stream<int>.fromIterable([1, 2, 3, 4, 5])) {
    if (i.isEven) {
      continue;
    }
    log.add(i);
