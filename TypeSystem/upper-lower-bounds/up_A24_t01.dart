...
void main() {
  ...
  // - R0 = UP(int, num) = num
  // - P20 = DOWN(X, X) = X
  // - P21 = DOWN(int, num) = int
  // - P22 = DOWN(int, num) = int
  var f0 = <X extends num>(X x, int y, {required int a}) => a;
  var f1 = <X extends num>(X x, num y, {required num a}) => a;

  var v1 = (1 > 2) ? f0 : f1;
  v1.expectStaticType<
      Exactly<num Function<X extends num>(X, int, {required int a})>>();

  // Named parameters are not required
  var f2 = <X extends num>(X x, num y, {int a = 0}) => a;
  var f3 = <X extends num>(X x, int y, {num a = 1}) => a;

  var v2 = (1 > 2) ? f2 : f3;
  v2.expectStaticType<
      Exactly<num Function<X extends num>(X, int, {int a})>>();

  // Both types of named parameters
  var f4 = <X extends num>(X x, num y, {required int a}) => a;
  var f5 = <X extends num>(X x, int y, {num a = 1}) => a;
