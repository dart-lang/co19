...
test1<X extends X Function<T>(int, [num]), Y extends Y Function<T>(int, [num])>(
  X x,
  Y y,
) {
  var v = 1 > 2 ? x : y;
  v.expectStaticType<Exactly<Object? Function<T>(int, [num])>>();
}

test2<
  X extends X Function<T extends num>(int, [num]),
  Y extends Y Function<T extends num>(int, [num])
>(X x, Y y) {
  var v = 1 > 2 ? x : y;
  v.expectStaticType<Exactly<Object? Function<T extends num>(int, [num])>>();
}

test3<
  X extends void Function<T extends num>(X, [X]),
  Y extends void Function<T extends num>(Y, [Y])
>(X x, Y y) {
  var v = 1 > 2 ? x : y;
  v.expectStaticType<Exactly<void Function<T extends num>(Never, [Never])>>();
}

test4<X extends void Function<T>(X, [X]), Y extends void Function<T>(Y, [Y])>(
  X x,
  Y y,
) {
  var v = 1 > 2 ? x : y;
  v.expectStaticType<Exactly<void Function<T>(Never, [Never])>>();
}
