...
test1<X extends (X, {X n}),Y extends (Y, {Y n})>(X x, Y y) {
  var v = 1 > 2 ? x : y;
  v.expectStaticType<Exactly<(Object?, {Object? n})>>();
}

test2<X extends (int, {num? n}),Y extends (int, {num? n})>(X x, Y y) {
  var v = 1 > 2 ? x : y;
  v.expectStaticType<Exactly<(int, {num? n})>>();
}
