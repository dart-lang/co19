...
///
/// @description Check that if `S` is `Future<T>` then the greatest closure of
/// `S` with respect to `L` is `Future<U>` where `U` is the greatest closure of
/// `T` with respect to `L`.
/// @author sgrekhov22@gmail.com

import '../../Utils/static_type_helper.dart';

test<X extends Future<X>, Y extends Future<Y>>(X x, Y y) {
  var v = 1 > 2 ? x : y;
  v.expectStaticType<Exactly<Future<Object?>>>();
}
