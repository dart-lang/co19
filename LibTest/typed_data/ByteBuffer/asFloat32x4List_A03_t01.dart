 ...
 * @description Checks that creating Float32x4List view with negative offset,
...
void check(ByteBuffer buffer) {
  int shift = (Float32x4List.BYTES_PER_ELEMENT == 16) ? 4 : 0;
  // creates Float32x4List view with negative offset
  Expect.throws(() {buffer.asFloat32x4List(-16);});
  // creates Float32x4List view with offset not divisible by sixteen
  Expect.throws(() {buffer.asFloat32x4List(1);});
  // creates Float32x4List view with negative length
  Expect.throws(() {buffer.asFloat32x4List(0, -8);});
  // creates Float32x4List view with 'offsetInBytes + length * 16' greater then
  // buffer length
  Expect.throws(() {buffer.asFloat32x4List(16, buffer.lengthInBytes >> shift);});
}
