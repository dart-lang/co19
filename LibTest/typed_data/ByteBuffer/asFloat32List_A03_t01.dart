 ...
 * @description Checks that creating Float32List view with negative offset,
...
void check(ByteBuffer buffer) {
  ...
  // creates Float32List view with negative offset
  Expect.throws(() {buffer.asFloat32List(-1);});
  // creates Float32List view with offset not divisible by four
  Expect.throws(() {buffer.asFloat32List(1);});
  // creates Float32List view with negative length
  Expect.throws(() {buffer.asFloat32List(0, -8);});
  // creates Float32List view with 'offsetInBytes + length * 4'
  // greater then buffer length
  Expect.throws(() {buffer.asFloat32List(4, buffer.lengthInBytes >> shift);});
}
