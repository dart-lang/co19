...
main() {
  ...
  File file =
      new File(Directory.current.path + Platform.pathSeparator + fileName);
  file.createSync();
  try {
    Expect.isTrue(FileSystemEntity.identicalSync(file.path, fileName));
    Expect.isTrue(FileSystemEntity.identicalSync(fileName, file.path));
  } finally {
    file.delete();
  }
}
