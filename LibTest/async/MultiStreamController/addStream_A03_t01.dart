...
/// to `false`.
///
/// @description Checks that if `cancelOnError` is `false` all errors are
/// forwarded and only a done event ends the `addStream`.
...
main() {
  ...
  var stream = Stream<int>.multi((controller) {
    controller.addStream(source.stream, cancelOnError: false).then((_) {
      controller.close();
    });
  });
  listen(stream);
