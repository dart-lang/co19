...
///
/// @description Checks that for an object pattern invocation keys with
/// different parent keys are invoked
...
String test1(List<C> list) {
  switch (list) {
    case [C(val: 1), C(val: 2)]: // Expect call .val, .val
      return "match-1";
    case [C(val: 1), C(val: _)]: // Expect no new calls
      return "match-2";
    default:
      return "no match";
  }
...
String test2(List<C> list) =>
  switch (list) {
    [C(val: 1), C(val: 2)] => "match-1",
    [C(val: 1), C(val: _)] => "match-2",
    _ => "no match"
  };

main() {
  final c = C(1);
  Expect.equals("match-2", test1([c, c]));
  Expect.equals("C(1).val;C(1).val;", c.log);
  c.clearLog();

  Expect.equals("match-2", test2([c, c]));
  Expect.equals("C(1).val;C(1).val;", c.log);
}
