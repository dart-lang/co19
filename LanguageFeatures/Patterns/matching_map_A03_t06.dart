...
main() {
  Expect.equals("no match", test1(MyMap<String, int>({"keyX": 0, "key2": 2})));
  Expect.equals("[key1];", log);
  log = "";
  Expect.equals("no match", test2(MyMap<String, int>({"keyX": 0, "key2": 2})));
  Expect.equals("[key1];", log);
  log = "";
  Expect.equals("no match", test3(MyMap<String, int>({"keyX": 0, "key2": 2})));
  Expect.equals("[key1];", log);
  log = "";

  Expect.throws(() {
    var <String, int>{"key1": _, "key2": _} =
        MyMap<String, int>({"keyX": 0, "key2": 2});
  });
  Expect.equals("[key1];", log);
  log = "";
  Expect.throws(() {
