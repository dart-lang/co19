...
/// the map does not match. Test that if `V` is nullable and `v[k] == null` then
...
main() {
  Expect.equals("no match", test1(MyMap<String, int?>({"key1": null, "keyX": 42})));
  Expect.equals("[key1];containsKey(key1);[key2];", log);
  log = "";
  Expect.equals("no match", test2(MyMap<String, int?>({"key1": null, "keyX": 42})));
  Expect.equals("[key1];containsKey(key1);[key2];", log);
  log = "";
  ...
  Expect.throws(() {
    var <String, int?>{"key1": _, "key2": _} =
        MyMap<String, int?>({"key1": null, "keyX": 42});
  });
  Expect.equals("[key1];containsKey(key1);[key2];", log);
  log = "";
  Expect.throws(() {
    final <String, int?>{"key1": _, "key2": _} =
        MyMap<String, int?>({"key1": null, "keyX": 42});
  });
  Expect.equals("[key1];containsKey(key1);[key2];", log);
}
