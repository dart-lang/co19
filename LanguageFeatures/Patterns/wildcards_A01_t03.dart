...
/// }
/// ```
/// @description Check that wildcard `_` can be used in set patterns
...
main() {
  var set1 = {1, 2, 3, 4};
  var {_, _two, _, __} = set1;
  Expect.equals(2, _two);
  Expect.equals(4, __);

  var set2 = {"1", 2, "3", 4};
  var {String _, two, _, num ___} = set;
  Expect.equals(2, two);
  Expect.equals(4, ___);

  Expect.throws(() {
    var {int _, _, _, num _} = set2;
  });
  Expect.throws(() {
    var {_, String _, _, num _} = set2;
  });
  Expect.equals("{_, 2, var x}", {1, 2, "3"});
  Expect.equals("{String _, _, _}", {"", 2, 3});
  Expect.equals("default", {0, 1, 2});
  Expect.equals("default", {"0", "1"});
