...
///
/// @description Checks that if `p` is empty and `l > 0` ('l' is length of the
/// `v`) then the match fails and no elements of `v` are accessed via '[]'
...
main() {
  ...
  Expect.throws(() {
    var <String, int>{} = MyMap<String, int>({"key1": 1});
  });
  Expect.equals("", log);
  Expect.throws(() {
    final <String, int>{} = MyMap<String, int>({"key1": 1});
  });
  Expect.equals("", log);
}
