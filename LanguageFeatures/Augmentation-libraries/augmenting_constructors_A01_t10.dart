...
import augment 'augmenting_constructors_A01_t10_lib.dart';
...
class C {
  var x, y;
  C(int this.x, [final int this.y = 0]);
  C.foo({required int this.x, final int this.y = 0});
  C.bar(int x, [final int y = 0]): x = x, y = y;
  C.baz({required int x, final int y = 0}): x = x, y = y;
}
...
extension type ET(int x) {
  ET.foo(int this.x, [final int y = 0]);
  ET.bar({required int this.x, final int y = 0});
  ET.baz(int x, [final int y = 0]): x = x;
  ET.qux({required int x, final int y = 0}): x = x;
}
