...
///      ('=' <expression>)?;
///
/// @description Check that required named parameters of the primary constructor
...
class C2({required var int x,});

enum E1({required int x,}) {
  e0(x: 1);
  final int x;
  this: x = x;
}

enum E2({required final int x,}) {
  e0(x: 2);
}

extension type ET({required int x,});

main() {
  Expect.equals(1, C1(x: 1).x);
  Expect.equals(2, C2(x: 2).x);
  Expect.equals(1, E1.e0.x);
  Expect.equals(2, E2.e0.x);
  Expect.equals(3, ET(x: 3).x);
}
