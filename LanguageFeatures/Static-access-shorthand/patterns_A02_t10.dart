...
/// map pattern it's treated as a normal constant expression, using the context
/// type it's given.
...
main() {
  String res = "";
  if ({"key": C(1)} case {"key": .one}) {
    res = "C Ok";
  }
  Expect.equals("C Ok", res);

  M m = MC(1);
  if ({"key": m} case {"key": .one}) {
    res = "M Ok";
  }
  Expect.equals("M Ok", res);

  if ({"key": E.e1} case {"key": .one}) {
    res = "E Ok";
  }
  Expect.equals("E Ok", res);

  if ({"key": E.e1} case {"key": .e1}) {
    res = "E Ok again";
  }
  Expect.equals("E Ok again", res);

  if ({"key": ET(1)} case {"key": .one}) {
    res = "ET Ok";
  }
